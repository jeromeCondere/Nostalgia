...
to setup
  ...
  ; ask patches with [sqrt ((pxcor * pxcor) + (pycor * pycor)) > g-radius-limit]
  ask patches with [abs (pxcor) > g-limit or abs (pycor) > g-limit]
  [
     set pcolor g-out-patch-color
  ]
...
to go
  ...
  ask turtles with [pcolor = g-out-patch-color]
  [
    hide-turtle
  ]
  tick
